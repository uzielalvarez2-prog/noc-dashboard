...
// Base editable de clientes con Contrato Marco (empresa/servicio/siglas).
// Análoga a /api/clientes-top pero alimenta ContratoMarco → syncContratoMarco.
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error("[GET /api/contrato-marco-clientes]", err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (Array.isArray(body.bulk)) {
      ...
      const result = await db.contratoMarco.createMany({ data: rows });
    ...
    if (dupOr.length > 0) {
      const existing = await db.contratoMarco.findFirst({ where: { OR: dupOr } });
      if (existing)
        return NextResponse.json(
          {
    ...
    const cliente = await db.contratoMarco.create({
      data: {
        siglasIm,
        company,
        serviceRef,
    ...
    console.error("[POST /api/contrato-marco-clientes]", err);
