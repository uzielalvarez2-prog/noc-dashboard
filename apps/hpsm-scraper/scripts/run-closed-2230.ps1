...
while ($true) {
    ...
    if ($lockWaited -ge $lockWaitMax) {
        Write-Host "$(Get-Date -Format 'HH:mm:ss') [run-closed-2230] HPSM ocupado (PID $lockedPid) tras ${lockWaitMax}s - abortado"
        Stop-Transcript; exit 1
    }
    Write-Host "$(Get-Date -Format 'HH:mm:ss') [run-closed-2230] HPSM ocupado (PID $lockedPid) - esperando..."
