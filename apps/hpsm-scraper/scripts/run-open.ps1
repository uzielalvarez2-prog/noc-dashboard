...
Start-Transcript -Path "$logDir\run-open-$(Get-Date -Format 'yyyyMMdd').log" -Append
...
if ($lockedPid -gt 0 -and (Get-Process -Id $lockedPid -ErrorAction SilentlyContinue)) {
    Write-Host "$(Get-Date -Format 'HH:mm:ss') [run-open] HPSM ocupado (PID $lockedPid) - saltado"
...
try {
    ...
    Write-Host "$(Get-Date -Format 'HH:mm:ss') [run-open] iniciando..."
    & $nodeExe $tsxCli src/run-open.ts
